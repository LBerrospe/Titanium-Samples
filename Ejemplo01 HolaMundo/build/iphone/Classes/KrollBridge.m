...
NSString * Ejemplo01HolaMundo$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation Ejemplo01HolaMundoObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_ejemplo01holamundo);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Ejemplo01HolaMundo global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Ejemplo01HolaMundo" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_ejemplo01holamundo = [[Ejemplo01HolaMundoObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_ejemplo01holamundo];
	
	NSString *_ejemplo01holamundoNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _ejemplo01holamundoNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_ejemplo01holamundo valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:Ejemplo01HolaMundo$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.ejemplo01holamundo.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.ejemplo01holamundo.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
