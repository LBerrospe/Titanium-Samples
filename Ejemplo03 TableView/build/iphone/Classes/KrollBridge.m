...
NSString * Ejemplo03TableView$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation Ejemplo03TableViewObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_ejemplo03tableview);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Ejemplo03TableView global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Ejemplo03TableView" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_ejemplo03tableview = [[Ejemplo03TableViewObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_ejemplo03tableview];
	
	NSString *_ejemplo03tableviewNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _ejemplo03tableviewNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_ejemplo03tableview valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:Ejemplo03TableView$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.ejemplo03tableview.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.ejemplo03tableview.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
