...
//Rare exceptions to indicate a bug in the _ejemplo03tableview code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_ejemplo03tableview = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_ejemplo03tableview]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.ejemplo03tableview.module"
